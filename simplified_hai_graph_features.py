...
"""
Simplified Graph-based Feature Engineering and Anomaly Detection for HAI-21.03 Dataset

This module provides a simplified approach with reduced model complexity but larger sliding window
for the HAI-21.03 industrial control system security dataset, focusing on balancing
computational efficiency with detection performance.
"""
...
class SimplifiedHAIGraphFeatures:
    """
    Simplified graph-based feature engineering and anomaly detection for HAI-21.03 dataset.
    
    This class provides a streamlined approach with reduced model complexity but larger sliding window
    for creating graph structures, performing feature engineering, and detecting anomalies.
    """
    
    def __init__(self, correlation_threshold=0.7, min_anomaly_length=25, gap_threshold=2):
        """
        Initialize the SimplifiedHAIGraphFeatures class.
    ...
    def calculate_graph_centrality(self, G=None):
        """
        Calculate basic centrality measures for nodes in the graph.
        ...
        # Calculate centrality measures (simplified to just degree and betweenness)
    ...
    def simplified_graph_based_feature_engineering(self, df, G=None):
        """
        ...
        # Add centrality-based features (simplified)
    ...
    def select_features(self, train_df, test_df, n_components=30, n_features=50):
        """
        Select features and reduce dimensions (simplified).
    ...
    def create_sequences(self, data, seq_length=128, stride=10):
        """
        Create sequences for time series models with larger window.
        
        Args:
            data (np.array): Input data
            seq_length (int): Sequence length (default 128)
    ...
    def build_simplified_lstm_autoencoder(self, seq_length, n_features):
        """
        ...
        # Encoder - simplified with just one LSTM layer
    ...
    def train_model(self, X_train_seq, epochs=50, batch_size=64, validation_split=0.2, patience=10, model_path=None):
        """
        Train the simplified LSTM autoencoder model.
        ...
        # Build model
        self.model = self.build_simplified_lstm_autoencoder(X_train_seq.shape[1], X_train_seq.shape[2])
    ...
    def determine_threshold(self, X_train_seq, method='dynamic', contamination=0.01, percentile=99):
        """
    ...
    def detect_anomalies(self, X_test_seq, seq_length, stride, data_length):
        """
        ...
        anomaly_labels = (anomaly_scores > 0.65).astype(int)
        
        # Apply post-processing
        anomaly_labels = self.post_process_anomalies(anomaly_labels)
        
        return anomaly_scores, anomaly_labels
    
    def post_process_anomalies(self, anomaly_labels):
        """
        ...
        while i < len(processed_labels):
            if processed_labels[i] == 1:
                ...
                if j < len(processed_labels) - self.gap_threshold:
                    next_start = j
                    while next_start < j + self.gap_threshold and next_start < len(processed_labels) and processed_labels[next_start] == 0:
                        next_start += 1
                    
                    if next_start < j + self.gap_threshold and next_start < len(processed_labels) and processed_labels[next_start] == 1:
    ...
    def run_simplified_anomaly_detection_pipeline(self, train_df, test_df, seq_length=128, stride=10, 
                                               n_components=30, n_features=50,
        ...
        Args:
            train_df (pd.DataFrame): Training dataframe
            test_df (pd.DataFrame): Test dataframe
            seq_length (int): Sequence length for time series (default 128)
            stride (int): Stride for sliding window
            n_components (int): Number of PCA components
            n_features (int): Number of top features to select
            epochs (int): Maximum number of epochs
        ...
        Returns:
            tuple: (anomaly_scores, anomaly_labels, y_test)
        """
        # Create graph structure
        print("Creating graph structure...")
        G = self.create_graph_from_columns(train_df)
        
        # Perform simplified graph-based feature engineering
        print("Performing simplified graph-based feature engineering...")
        train_df_processed = self.simplified_graph_based_feature_engineering(train_df, G)
        test_df_processed = self.simplified_graph_based_feature_engineering(test_df, G)
        
        # Select features and reduce dimensions
        print("Selecting features and reducing dimensions...")
        X_train_pca, X_test_pca, y_test, _, _, _ = self.select_features(
            train_df_processed, test_df_processed, n_components, n_features
        )
        ...
        # Train model
        print("Training simplified LSTM autoencoder...")
        self.train_model(X_train_seq, epochs, batch_size, validation_split, patience, model_path)
        
        # Determine threshold
        print("Determining threshold...")
        self.determine_threshold(X_train_seq, method='dynamic')
        
        # Detect anomalies
        print("Detecting anomalies...")
        anomaly_scores, anomaly_labels = self.detect_anomalies(
            X_test_seq, seq_length, stride, len(y_test)
        )
        
        return anomaly_scores, anomaly_labels, y_test